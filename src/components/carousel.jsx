            ...
            <div className='embla__viewport' ref={emblaRef}>
                <div className='embla__container'>
                    {slides.map((slide,index) => (
                        <div key={`${slide.title}-${index}`} className='rounded-4xl m-[var(--slide-spacing)] max-w-[250px]  flex-none  transform-gpu relative'>
                            <div className='absolute z-10 h-full w-full flex flex-col rounded-2xl pb-10 justify-end'
                            style={{
                                background: 'linear-gradient(rgba(245,245,245,0) 34.08%, rgb(34,48,74) 100%)'
                            }}></div>
                            <div className='absolute z-10 p-3 h-[105%] flex justify-end flex-col'>
                                <div className='text-white font-mulish'>                            
                                    <h3 className='font-bold text-lg'>{slide.title}</h3>
                                    <p className='text-xs text-gray-300'>{slide.location}</p>
                                    <p className='text-xs'>{slide.description}</p>
                                    <a href={slide.link}>
                                        <button className="py-3 px-8 mt-3 uppercase font-extrabold bg-button rounded-lg text-xs text-text hover:cursor-pointer hover:scale-105 transition-all font-mulish">
                                            Ride Details
                                        </button>
                                    </a>
                                </div>
                            </div>
                            <video key={slide.media} autoPlay muted playsInline loop preload='metadata' className='aspect-[229/394] rounded-3xl w-full object-cover lg:aspect-[246/346]'>
                ...
                <div className='embla__controls'>
                    <div className='absolute -top-12 right-8 flex gap-4'>
                        <button onClick={scrollPrev} className='px-4.5 py-3.5 hover:scale-105 transition-all transform -translate-1/2 bg-button cursor-pointer rounded-full' style={{transition:'all 200ms'}}> 
                        ...
                        <button onClick={scrollNext} className='px-4.5 py-3.5 transform hover:scale-105 -translate-1/2 bg-button rounded-full cursor-pointer' style={{transition:'all 200ms'}}> 
                            <svg width="13" height="20" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 13 20">
                                <path fill="#334DCF" 
